...
<div class="px-4 pt-5 pb-10 space-y-5">
    ...
    <form method="GET" action="{{ route('admin.sections.contents.index', $section) }}" class="flex flex-wrap items-center gap-2">
        ...
        <select name="type" class="rounded-full border border-slate-200 bg-white px-3 py-2 text-xs focus:border-indigo-400 focus:outline-none focus:ring-1 focus:ring-indigo-400">
        ...
        @forelse($contents as $content)
            ...
            <div class="rounded-2xl bg-white border border-slate-100 shadow-sm">
                ...
                <div class="flex items-start gap-3 px-4 pt-3.5 pb-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex flex-wrap items-center gap-1.5 mb-0.5">
                            <p class="text-sm font-bold text-slate-800 break-words">{{ $content->title }}</p>
                            <span class="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold {{ $typeColor }}">
                                {{ $typeLabel }}
                            </span>
                            <span class="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold {{ $content->is_active ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500' }}">
                        ...
                        @elseif($content->body)
                            <p class="text-xs text-slate-400 line-clamp-2">
                                {{ strip_tags(html_entity_decode($content->body, ENT_QUOTES | ENT_HTML5, 'UTF-8')) }}
                            </p>
                        @else
                            <p class="text-xs text-slate-300 italic">Belum ada isi konten.</p>
                        @endif
                ...
                <div class="flex items-center gap-2 overflow-x-auto px-4 py-2.5" style="scrollbar-width:none;-webkit-overflow-scrolling:touch">
                    ...
                    <form method="POST" action="{{ route('admin.sections.contents.toggle-active', [$section, $content]) }}" class="shrink-0">
                        @csrf
                        <button type="submit"
                            class="inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-xs font-semibold transition {{ $content->is_active ? 'bg-orange-50 text-orange-500 active:bg-orange-100' : 'bg-green-50 text-green-600 active:bg-green-100' }}">
                            {{ $content->is_active ? 'Non-aktifkan' : 'Aktifkan' }}
                        </button>
                    </form>

                    <form method="POST"
                          action="{{ route('admin.sections.contents.destroy', [$section, $content]) }}"
                          class="shrink-0"
