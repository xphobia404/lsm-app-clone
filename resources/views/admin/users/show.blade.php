    ...
    <div class="mb-5">
        <div class="mb-2 flex items-center justify-between">
            ...
            <a href="{{ route('admin.users.edit', $user) }}"
               class="text-xs font-medium text-indigo-600">Kelola &rarr;</a>
        </div>
        ...
        @if($user->learningSchemas->isEmpty())
            <div class="rounded-2xl border border-dashed border-slate-300 bg-white px-4 py-6 text-center">
                <p class="text-xs text-slate-400">Belum ada materi yang terhubung ke user ini.</p>
            </div>
        @else
            <div class="space-y-2">
                @foreach($user->learningSchemas as $schema)
                    <div class="rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <p class="text-sm font-semibold text-slate-700">{{ $schema->title }}</p>
                                <p class="text-[11px] text-slate-400">
                                    {{ $schema->sections_count }} section
                                </p>
                            </div>

                            @php
                                $enrollmentStatus = $schema->pivot->status ?? 'active';
                            ...
                            <span class="shrink-0 rounded-full px-2.5 py-1 text-[10px] font-semibold {{ $enrollmentClass }}">
                                {{ ucfirst($enrollmentStatus) }}
                            </span>
                        </div>

                        @if($schema->pivot->enrolled_at)
                            <p class="mt-2 text-[11px] text-slate-400">
                                Enrolled: {{ \Illuminate\Support\Carbon::parse($schema->pivot->enrolled_at)->format('d M Y H:i') }}
                            </p>
                        @endif
                    </div>
                @endforeach
        ...
        @else
            <div class="space-y-2">
                @foreach($sectionsWithStatus as $section)
                    @php
                        $statusClass = $section->progress_status === 'completed'
                            ? 'bg-emerald-100 text-emerald-700'
                            : 'bg-slate-100 text-slate-500';

                        $statusLabel = $section->progress_status === 'completed'
                            ? 'Completed'
                            : 'Not Completed';
                    @endphp
        ...
        @else
            <div class="space-y-2">
                @foreach($user->progresses as $progress)
                    @php
                        $badgeClass = ($progress->status ?? null) === 'completed'
                            ? 'bg-emerald-100 text-emerald-700'
                            : 'bg-slate-100 text-slate-500';

                        $badgeLabel = ($progress->status ?? null) === 'completed'
                            ? 'Completed'
                            : 'Not Completed';
                    @endphp
                    ...
                    <div class="rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <span class="shrink-0 rounded-full px-2.5 py-1 text-[10px] font-semibold {{ $badgeClass }}">
                                {{ $badgeLabel }}
                            </span>
                        </div>
                    </div>
