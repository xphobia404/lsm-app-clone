...
<div class="px-4 pt-5 pb-10">
    ...
    <div class="mb-5 flex items-center gap-2 text-xs text-slate-400">
        ...
        <a href="{{ route('admin.learning-schemas.sections.index', $learningSchema) }}" class="hover:text-indigo-600 transition truncate max-w-[140px]">{{ $learningSchema->title }}</a>
        <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
        <span class="text-slate-600 font-medium">Tambah Section</span>
    </div>

    <div class="max-w-xl">
        <div class="rounded-2xl bg-white border border-slate-100 shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-100">
                <h2 class="text-sm font-bold text-slate-800">Tambah Section Baru</h2>
                <p class="text-xs text-slate-400 mt-0.5">Di dalam schema: <span class="font-medium text-slate-600">{{ $learningSchema->title }}</span></p>
            </div>
            <form method="POST" action="{{ route('admin.learning-schemas.sections.store', $learningSchema) }}" class="px-5 py-5 space-y-4">
                @csrf
                @include('admin.sections._form', ['section' => null, 'nextOrder' => $nextOrder])
                <div class="flex items-center gap-3 pt-1">
                    ...
                    <a href="{{ route('admin.learning-schemas.sections.index', $learningSchema) }}"
