    ...
    <div class="max-w-xl">
        <div class="rounded-2xl bg-white border border-slate-100 shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-100">
                <h2 class="text-sm font-bold text-slate-800">Edit Learning Schema</h2>
                <p class="mt-0.5 text-xs text-slate-400 truncate">{{ $learningSchema->title }}</p>
            </div>
            <form method="POST" action="{{ route('admin.learning-schemas.update', $learningSchema) }}" class="px-5 py-5 space-y-4">
                @csrf @method('PUT')
