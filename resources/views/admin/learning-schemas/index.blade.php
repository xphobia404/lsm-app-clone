                ...
                <div class="px-4 py-3.5 flex items-start justify-between gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            <p class="text-sm font-bold text-slate-800 truncate">{{ $ls->title }}</p>
                        ...
                        @if($ls->description)
                            <p class="mt-0.5 text-xs text-slate-400 line-clamp-2">{{ $ls->description }}</p>
                        @endif
                        <div class="mt-1.5 flex items-center gap-3">
                            ...
                            <span class="text-xs text-slate-400">Dibuat {{ $ls->created_at->diffForHumans() }}</span>
                        </div>
                    </div>
                    ...
                    <div class="flex shrink-0 flex-wrap items-center gap-1.5">
                        <a href="{{ route('admin.learning-schemas.sections.index', $ls) }}"
                           class="inline-flex items-center gap-1 rounded-full bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-600 hover:bg-indigo-100 active:bg-indigo-200 transition">
                        ...
                        <a href="{{ route('admin.learning-schemas.edit', $ls) }}"
                           class="inline-flex items-center gap-1 rounded-full bg-amber-50 px-3 py-1.5 text-xs font-semibold text-amber-600 hover:bg-amber-100 active:bg-amber-200 transition">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        <form method="POST" action="{{ route('admin.learning-schemas.toggle-active', $ls) }}">
                            ...
                            <button type="submit"
                                class="inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-xs font-semibold transition
                                    {{ $ls->is_active ? 'bg-orange-50 text-orange-500 hover:bg-orange-100' : 'bg-green-50 text-green-600 hover:bg-green-100' }}">
                        ...
                        <form method="POST" action="{{ route('admin.learning-schemas.destroy', $ls) }}"
                            ...
                            @csrf @method('DELETE')
                            <button type="submit"
                                class="inline-flex items-center gap-1 rounded-full bg-red-50 px-3 py-1.5 text-xs font-semibold text-red-500 hover:bg-red-100 active:bg-red-200 transition">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
