<x-app-layout :title="'Quiz — ' . $section->title">
    ...
    <div class="sticky top-[57px] z-30 bg-white border-b border-slate-100 px-4 py-2">
        ...
        <div class="h-1.5 w-full rounded-full bg-slate-100 overflow-hidden">
            <div id="progress-bar" class="h-full rounded-full bg-blue-600 transition-all duration-300"
                 style="width: calc(1 / {{ $quizzes->count() }} * 100%)"></div>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('user.quiz.submit', $section) }}" id="quiz-form">
        ...
        <div class="px-4 pt-5 pb-32">
            ...
            <div class="quiz-slide {{ $i === 0 ? '' : 'hidden' }}" data-index="{{ $i }}">
                ...
                <div class="mb-4 flex items-center gap-2">
                    <span class="flex h-7 w-7 items-center justify-center rounded-full bg-blue-600 text-xs font-bold text-white">{{ $i + 1 }}</span>
                    <span class="text-xs text-slate-400 font-medium uppercase tracking-wide">Pilihan Ganda</span>
                </div>

                <p class="mb-3 text-sm font-semibold text-slate-800 leading-relaxed">{{ $quiz->question }}</p>

                {{-- Media soal (gambar, video, audio) --}}
                @if($quiz->media->isNotEmpty())
                <div class="mb-4 space-y-2">
                    @foreach($quiz->media as $m)
                        @if($m->type === 'image')
                        <img src="{{ $m->url }}" alt="Media soal {{ $i+1 }}"
                             class="w-full max-h-60 rounded-2xl border border-slate-200 object-contain bg-slate-50">
                        @elseif($m->type === 'video')
                        <video src="{{ $m->url }}" controls
                               class="w-full max-h-60 rounded-2xl border border-slate-200 bg-black"></video>
                        @elseif($m->type === 'audio')
                ...
                <div class="space-y-3">
                    ...
                    <label for="{{ $inputId }}"
                        ...
                        <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 border-slate-300 text-xs font-bold uppercase">{{ $key }}</span>
                        <input type="radio" id="{{ $inputId }}" name="answers[{{ $quiz->id }}]" value="{{ $key }}"
                               class="sr-only" data-question-index="{{ $i }}"
        ...
        <div class="fixed bottom-[72px] left-0 right-0 z-40 bg-white/95 backdrop-blur border-t border-slate-100 px-4 py-3">
            <div class="flex items-center gap-3">
                <button type="button" id="btn-prev" onclick="navigate(-1)" disabled
            ...
            <div class="mt-2.5 flex items-center justify-center gap-1.5">
                @foreach($quizzes as $i => $quiz)
                <button type="button" onclick="goTo({{ $i }})" data-index="{{ $i }}"
                        class="dot h-2 w-2 rounded-full transition-all duration-200 {{ $i === 0 ? 'bg-blue-600 w-4' : 'bg-slate-300' }}"></button>
                @endforeach
            </div>
        </div>
    </form>
    ...
    function onAnswered(i) { answered[i] = true; updateDot(i); }
    function goTo(index) {
        ...
        document.querySelectorAll('.dot').forEach((dot, i) => {
            ...
            dot.classList.add(i===index?'bg-blue-600':answered[i]?'bg-green-500':'bg-slate-300');
            if(i===index) dot.classList.add('w-4');
        });
        ...
        const btnNext=document.getElementById('btn-next'), btnSubmit=document.getElementById('btn-submit');
        if(current===total-1){btnNext.classList.add('hidden');btnSubmit.classList.remove('hidden');btnSubmit.classList.add('flex');}
        else{btnNext.classList.remove('hidden');btnSubmit.classList.add('hidden');btnSubmit.classList.remove('flex');}
    }
    function updateDot(i){const d=document.querySelector(`.dot[data-index="${i}"]`);if(d&&i!==current){d.classList.remove('bg-slate-300','bg-blue-600','w-4');d.classList.add('bg-green-500');}}
    function navigate(dir){const n=current+dir;if(n>=0&&n<total)goTo(n);}
    goTo(0);
    </script>
