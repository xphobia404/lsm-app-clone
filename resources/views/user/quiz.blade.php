...
<div class="px-4 pt-5 pb-10">
    ...
    {{-- Header --}}
    <div class="mb-4 rounded-2xl bg-gradient-to-br from-amber-400 to-orange-500 p-4 text-white shadow-md">
    ...
    {{-- Result flash (jika ada redirect balik) --}}
    @if(session('quiz_result'))
    @php $result = session('quiz_result'); @endphp
    <div class="mb-4 rounded-2xl p-4 {{ $result['passed'] ? 'bg-emerald-50 border border-emerald-200' : 'bg-red-50 border border-red-200' }}">
        <div class="flex items-center gap-2">
            @if($result['passed'])
            <span class="text-xl">🎉</span>
            <div>
                <p class="text-sm font-bold text-emerald-700">Selamat! Kamu lulus.</p>
                <p class="text-xs text-emerald-600">Skor: {{ $result['score'] }}% &bull; {{ $result['correct'] }}/{{ $result['total'] }} benar</p>
            </div>
            @else
            <span class="text-xl">😔</span>
            <div>
                <p class="text-sm font-bold text-red-700">Belum lulus, coba lagi.</p>
                <p class="text-xs text-red-500">Skor: {{ $result['score'] }}% &bull; {{ $result['correct'] }}/{{ $result['total'] }} benar</p>
            </div>
            @endif
        </div>
    </div>
    ...
    <form method="POST"
            ...
            <div class="rounded-2xl bg-white border border-slate-100 shadow-sm overflow-hidden"
                ...
                <div class="flex items-center gap-2.5 border-b border-slate-100 px-4 py-2.5 bg-slate-50">
                    <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full
                                 bg-amber-100 text-xs font-bold text-amber-700">{{ $i + 1 }}</span>
                ...
                <div class="p-4 space-y-3">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="rounded-xl overflow-hidden border border-slate-100">
                            ...
                            <img src="{{ Storage::url($media->file_path) }}"
                                 alt="{{ $media->title ?: 'Gambar soal ' . ($i + 1) }}"
                                 loading="lazy"
                            ...
                            @endif
                            @if($media->title)
                            <p class="px-3 py-1.5 text-xs text-slate-500 bg-slate-50">{{ $media->title }}</p>
                            @endif
                        </div>
                        @break
                        ...
                        <div class="rounded-xl border border-slate-100 bg-slate-50 px-3 py-2.5">
                            ...
                            <p class="mt-1 text-xs text-slate-400">{{ $media->description }}</p>
                            @endif
                        </div>
                        @break
                        ...
                        <a href="{{ $media->url }}" target="_blank" rel="noopener noreferrer"
                           class="flex items-center gap-2.5 rounded-xl border border-indigo-100 bg-indigo-50 px-3 py-2.5
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-xs font-semibold text-indigo-700 truncate">
                                    {{ $media->title ?: 'Buka referensi' }}
                                </p>
                    ...
                    <div class="space-y-2" id="options-{{ $quiz->id }}">
                        ...
                        <label class="quiz-option flex items-center gap-3 rounded-xl border border-slate-200 px-3 py-2.5
                                      cursor-pointer transition active:bg-indigo-50"
                               data-qid="{{ $quiz->id }}">
                            <input type="radio" name="answers[{{ $quiz->id }}]" value="{{ $key }}"
                                   class="hidden quiz-radio"
                                   data-qid="{{ $quiz->id }}">
                            <span class="option-indicator flex h-7 w-7 shrink-0 items-center justify-center rounded-full
                                          border-2 border-slate-200 text-xs font-bold text-slate-400 uppercase
                                          transition">
                                {{ $key }}
                            </span>
        ...
        <div class="mt-6 space-y-2">
            <button type="submit" id="btn-submit"
                    class="w-full rounded-2xl bg-indigo-600 py-3.5 text-sm font-bold text-white shadow-md
                           active:bg-indigo-700 transition disabled:opacity-50">
                Kumpulkan Jawaban
            </button>
...
(function () {
    ...
    function updateProgress() {
        ...
        hint.textContent = count < total
            ? (total - count) + ' soal belum dijawab'
            : '✓ Semua soal sudah dijawab';
        hint.className = count < total
